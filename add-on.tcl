...
proc dcc:adduser {hand idx paras} {
	...
	*dcc:adduser $hand $idx $paras
	if {[validuser $user]} {
        ...
        putserv "PRIVMSG $user :Ti si dobaven(adnat) v men ot $hand."
        putserv "PRIVMSG $user :Molq setni si pass che vse oshte nqmash po slednia nachin -> /msg me pass <your-password>"
    ...
    if {[validuser $user]} {
        ...
        *dcc:+user $hand $idx $paras
        if {[validuser $user]} {
            setuser $user xtra ADDED "by $hand as $user ([strftime %d-%m-%Y@%H:%M])"
		tellaboutnew $hand $user
        putserv "PRIVMSG $user :Ti si dobaven(adnat) ot $hand."
        putserv "PRIVMSG $user :Molq setni si pass v men che vse oshte nqmash po slednia nachin -> /msg me pass <your-password>"
        putlog "Messages to $user was succifuly send."
        }
    }
}
