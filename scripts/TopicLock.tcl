...
set foundchan 0
	if {$hand != "*"} {
		...
		if {!$foundchan} {
			putquick "NOTICE $nick :Login unsuccessful - You need to be on same channel with me."
			putlog "topiclock: $nick ($uhost) tried to log in as $hand - Not on any channel."
		} elseif {$pass == ""} {
			putquick "NOTICE $nick :Usage: /msg $botnick LOGIN <password>"
			putlog "topiclock: $nick ($uhost) tried to log in as $hand - No password."
		} elseif {[getuser $hand XTRA "LOGIN"] == "1"} {
			putquick "NOTICE $nick :Login unsuccessful - You have already logged in."
			...
			putquick "NOTICE $nick :Login unsuccessful - Wrong password."
			putlog "topiclock: $nick ($uhost) tried to log in as $hand - Wrong password."
		}
	} else {
		putquick "NOTICE $nick :Login unsuccessful - You're not on my userlist."
...
proc msg:tl_logout {nick uhost hand arg} {
	if {$hand != "*"} {
		if {[getuser $hand XTRA "LOGIN"] == "1"} {
			...
			putquick "NOTICE $nick :Successfully logged out."
			putlog "topiclock: $nick ($uhost) logged out as $hand."
		} else {
			putquick "NOTICE $nick :Logout unsuccessful - You're already logged out."
			putlog "topiclock: $nick ($uhost) tried to log out as $hand - Already logged out."
		}		
	} else {
		putquick "NOTICE $nick :Logout unsuccessful - You're not on my userlist."
...
if {$tl_login} { if {![tl_login_check $hand]} { return 0 } }
	...
	if {![tl_botonchan $chan]} {
		putquick "NOTICE $nick :I'm not on $chan."
	} else { 
...
if {[lindex [split $arg] 0] != ""} { set chan [lindex [split $arg] 0] }
	if {![tl_botonchan $chan]} {
		...
		if {[info exists tl_lock([string tolower $chan])] && ($tl_lock([string tolower $chan]))} { 
			...
			putquick "NOTICE $nick :Unholding topic of $chan."
		} else {
			putquick "NOTICE $nick :I'm not holding topic of $chan."
		}
	}
return 1
}
...
set topic [join [lrange [split $arg] 0 end]]
	...
	if {$chanlist != ""} {
		if {[llength $chanlist] == 1} {
			putquick "NOTICE $nick :Holding topic of [join $chanlist ", "] ([llength $chanlist] channel)"
		} else {
			putquick "NOTICE $nick :Holding topic of [join $chanlist ", "] ([llength $chanlist] channels)"
		}
	} else {
		putquick "NOTICE $nick :Holding topic of 0 channels."
	}
return 1
}
...
set chanlist ""
	...
	if {$chanlist != ""} {
		if {[llength $chanlist] == 1} {
			putquick "NOTICE $nick :Unholding topic of [join $chanlist ", "] ([llength $chanlist] channel)"
		} else {
			putquick "NOTICE $nick :Unholding topic of [join $chanlist ", "] ([llength $chanlist] channels)"
		}
	} else {
		putquick "NOTICE $nick :Unholding topic of 0 channels."
	}
return 1
}
...
set command [lindex [split $arg] 0]
	...
	switch -exact -- [string tolower $command] {
		"" {
			putquick "PRIVMSG $nick :topiclock.tcl v$tl_ver commands:"
			putquick "PRIVMSG $nick :  ${tl_cmdpfix}tlock          ${tl_cmdpfix}tunlock"
			putquick "PRIVMSG $nick :  ${tl_cmdpfix}tlockall       ${tl_cmdpfix}tunlockall"
			putquick "PRIVMSG $nick :To get more help on individual commands, type: '${tl_cmdpfix}tlhelp <command>'"
		}
		"tlock" {
			putquick "PRIVMSG $nick :###  ${tl_cmdpfix}tlock \[channel\] \[topic\]"
			putquick "PRIVMSG $nick :   will hold the topic of the channel you"
			putquick "PRIVMSG $nick :   have specified or your current channel."
		}
		"tunlock" {
			putquick "PRIVMSG $nick :###  ${tl_cmdpfix}tunlock \[channel\]"
			putquick "PRIVMSG $nick :   will unhold the topic of the channel you"
			putquick "PRIVMSG $nick :   have specified or your current channel."
		}
		"tlockall" {
			putquick "PRIVMSG $nick :###  ${tl_cmdpfix}tlockall \[topic\]"
			putquick "PRIVMSG $nick :   will hold topics of all channels."
		}
		"tunlockall" {
			putquick "PRIVMSG $nick :###  ${tl_cmdpfix}tunlockall"
			putquick "PRIVMSG $nick :   will unhold topics of all channels."
		}
		"default" { 
			putquick "PRIVMSG $nick :No help available on '$command'."
		}
	}
return 1
}
...
set topic [join [lrange [split $arg] 1 end]]
	if {$chan == ""} {
		putquick "NOTICE $nick :Usage: tlock <channel> \[topic\]"
	} else {
		if {![tl_botonchan $chan]} {
			putquick "NOTICE $nick :I'm not on $chan."
		} else { 
...
set chan [lindex [split $arg] 0]
	if {$chan == ""} {
		putquick "NOTICE $nick :Usage: tunlock <channel>"
	} else {
		if {![tl_botonchan $chan]} {
			putquick "NOTICE $nick :I'm not on $chan."
		} else { 
...
set command [lindex [split $arg] 0]
	switch -exact -- [string tolower $command] {
		"" {
			putquick "PRIVMSG $nick :topiclock.tcl v$tl_ver commands:"
			putquick "PRIVMSG $nick :  tlock          tunlock"
			putquick "PRIVMSG $nick :  tlockall       tunlockall"
			putquick "PRIVMSG $nick :To get more help on individual commands, type: 'tlhelp <command>'"
		}
		"tlock" {
			putquick "PRIVMSG $nick :###  tlock <channel> \[topic\]"
			putquick "PRIVMSG $nick :   will hold the topic of the channel you"
			putquick "PRIVMSG $nick :   have specified or your current channel."
		}
		"tunlock" {
			putquick "PRIVMSG $nick :###  tunlock <channel>"
			putquick "PRIVMSG $nick :   will unhold the topic of the channel you"
			putquick "PRIVMSG $nick :   have specified or your current channel."
		}
		"tlockall" {
			putquick "PRIVMSG $nick :###  tlockall \[topic\]"
			putquick "PRIVMSG $nick :   will hold topics of all channels."
		}
		"tunlockall" {
			putquick "PRIVMSG $nick :###  tunlockall"
			putquick "PRIVMSG $nick :   will unhold topics of all channels."
		}
		"default" { 
			putquick "PRIVMSG $nick :No help available on '$command'."
	...
	if {([string tolower $nick] != [string tolower $botnick]) && ($nick != "*") && ($uhost != "*")} {
		...
		if {($tl_fludprot) && ([info exists tl_lock([string tolower $chan])]) && ($tl_lock([string tolower $chan]))} {
			...
			if {(!$tl_flooded([string tolower $chan])) && ($tl_topiccount([string tolower $chan]) > [lindex $tl_maxtopics 0])} {
				...
				if {($tl_kick) && ([botisop $chan]) && ([onchan $nick $chan])} { 
					...
					putquick "KICK $chan $nick :$tl_kickreason" 
				}
				if {$dowhat != ""} {
		...
		if {([info exists tl_lock([string tolower $chan])]) && (([info exists tl_topic([string tolower $chan])]) && ($tl_lock([string tolower $chan]))) && ($topic != $tl_topic([string tolower $chan]))} { 
			if {([botisop $chan]) || (![string match "*t*" [lindex [getchanmode $chan] 0]])} {	
				putquick "TOPIC $chan :$tl_topic([string tolower $chan])"
				if {$tl_notice} { 
					putquick "NOTICE $nick :The topic of $chan is locked." 
					putquick "mode $chan -o $nick"
				}
			}
		}
...
putlog "Инсталиран: topiclock.tcl"
if {$tl_fludprot} { 
	putlog " `- Flood защитата е: активна. (Приети [lindex $tl_maxtopics 0] теми в [lindex $tl_maxtopics 1] секунди)" 
} else {
	putlog " `- Flood защитата е: неактивна." 
}