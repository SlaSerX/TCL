#         Script : Channel Guardian v1.02 by David Proper (Dr. Nibble [DrN])
...
#                  07/13/2003 - v1.02
#                              o Optional actions against OPs
#                              o Fixed color_spaces variable error.
#                                (Reported by Soulviva@EFnet)
#
#
#   Future Plans : Fix Bugs. :)
#                 o List of channels to protect (LordDark)
...
# Set this to 1 to ignore idiot OPs that might trigger the script or 0
# to bitch slap them like everyone else when they do stupid stuff.
set guard(ingoreops) 1
...
if {[string length $text] == 1} {incr g_1flood} else {set g_1flood 0}
 ...
 if {$g_spaces == $guard(spaces_warn)} {guardsay $nick $desc "$nick: $guard(spaces_reason)"}
