## Choose what type of ban you want, from the following.
set type_ban "20"

#### (I've decided that n?c?* | *n?c?* | *n?c? | *i?e?t* is exremely dangerous,
#### so I will NOT add them, if you thinks it is needed, mail me!)
# Types are:
## 1 - nick!*@*
## 2 - nick*!*@*
## 3 - *nick*!*@*
...
## i.e "# www http"
set forbidden "# #&%#&"
set whole_forb "join in|elate w|oWnedaNdScReWiNg"

## To ban when a user <B> MESSAGEs </B> me with the word(s). The second is
## for phrases.
set msg_forb "# www http .com .net .bg .org"
set whole_msg "click|visit|ela v|pls|click|vij|join|wlez|vles|come|see|copy at http|come to http|please join #|join #|elate #|go to #|visit www.|visit #|Get it at http|follow me to*#|Join pls #|freeservers.com|/server"

## To ban when a user says <B> PUBLIC </B> the word(s). The second is for
## phrases.
set pub_forb "# #&%#& LSD"
set whole_pub "join in|#&%#&|please join #|oWnedaNdScReWiNg|join #|elate #|go to #|visit www.|visit #|Get it at http|/server|follow me to*#|Join pls#|freeservers.com"


## To ban when a user <B> NOTICEs </B> me with the word(s). The second is for
## phrases.
set notc_forb "#"
set whole_notc "join|com|www|http|I'm screamming|LSD ; LSD ; LSD|click|visit|ela v|pls|click|vij|join|wlez|vles|come|see|copy at http|come to http|please join #|join #|elate #|go to #|visit www.|visit #|Get it at http|follow me to*#|Join pls #|freeservers.com|/server|oWnedaNdScReWiNg"

## To ban when a user <B> QUITs </B> with the word(s). The second is for 
## phrases.
set quit_forb "#"
set whole_quit "www|come to|click"

## To ban when a user <B> CTCPs </B> me with the word(s). The second is for
## phrases.
set ctcp_forb "prodavam"
set ctcp_forb "rent"
set ctcp_forb "Prodavam"
set ctcp_forb "PRODAVAM"
set ctcp_forb "Rent"
set ctcp_forb "RENT"
set whole_ctcp "prodavam"
set whole_ctcp "Rent"

## To ban when a user <B> CTCP REPLYs </B> me with the word(s). The second is
## for phrases.
set ctcr_forb "#"
set whole_ctcr "part my|join my|www|http|#"
...
## To ban when a user <B> KICKs </B> someone of a channel with the word(s).
## The second is for phrases.
set kick_forb "#"
set whole_kick "Take Over"

## Set this to the reason, why it is kick banned!
set reason "Spamming/Selling is NOT allowed in here!!!"
...
# Proc for kick reason.
proc invite_kick {nick uhost hand chan targ arg} {
    global botnick reason time bk k kb flags forbidden kick_forb whole_kick whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
    ...
    if {$goon == 1} {
	    ...
	    if {[onchan $nick $chan] == 1} {
	        if {$kb == 1} { kb $nick $chan $ban }
		if {$bk == 1} { bk $nick $chan $ban }
		if {$k  == 1} { k  $nick $chan $ban }
	    }
	}
    }
}
...
# Proc for topic change.
proc invite_topc {nick uhost hand chan arg} {
    global botnick reason time bk k kb flags forbidden topc_forb whole_topc whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
# Proc for msg invites.
proc invite_msg {nick uhost hand arg} {
    global botnick reason time bk k kb flags forbidden msg_forb whole_msg whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
# Proc for notice invites.
proc invite_notc {nick uhost hand arg dest} {
    global botnick reason time bk k kb flags forbidden notc_forb whole_notc whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
# Proc for CTCP REPLY invites.
proc invite_ctcr {nick uhost hand dest keyword args} {
    set arg "$keyword [lindex [split [string trimleft $args \{] "\}"] 0]"
    global botnick reason time bk k kb flags forbidden ctcr_forb whole_ctcr whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
# Proc for CTCP invites.
proc invite_ctcp {nick uhost hand dest keyword args} {
    set arg "$keyword [lindex [split [string trimleft $args \{] "\}"] 0]"
    global botnick reason time bk k kb flags forbidden ctcp_forb whole_ctcp whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
# Proc for QUIT invites.
proc invite_quit {nick uhost hand chan arg} {
    global botnick reason time bk k kb flags forbidden quit_forb whole_quit whole_forb
    if {[matchattr $hand $flags] == 1} {
	return
    }
    set goon 0
...
putlog "Loaded:Anti-Invite/Spam/Advertice.tcl"
