...
global botnick protflag exceflag deoptime msgeuser
 ...
 if {[botisop $chan]} {
   ...
   flushmode $chan
     if {[matchattr $hand |o $chan]} {
       chattr $hand |-ao+d $chan
       putserv "KICK $chan $nick :Don't deop my master! You have lost your op on $chan for that !!"
       if {$msgeuser} {putserv "PRIVMSG $nick :If you want your op back please talk to one of my masters. Masters on channel $chan are now: [chanlist $chan m|m]"}
     } else {
       if {[validuser $hand]} {
       chattr $hand |-ao+d $chan
       }
       putserv "KICK $chan $nick :Don't deop my master!"
...
global botnick protflag exceflag kicktime msgeuser
 ...
 if {[botisop $chan]} {
   ...
   flushmode $chan
     if {[matchattr $hand |o $chan]} {
       chattr $hand |-ao+d $chan
       putserv "KICK $chan $nick :Ne se ebavai s $nick!"
       if {$msgeuser} {putserv "PRIVMSG $nick :If you want your op back please talk to one of my masters. Masters on channel $chan are now: [chanlist $chan m|m]"}
     } else {
       if {[validuser $hand]} {
       chattr $hand |-ao+d $chan
   ...
   if {[string match "$ban" "$botname"]} {
    ...
    flushmode $chan
     if {[matchattr $hand |o $chan]} {
       ...
       putserv "KICK $chan $nick :Do not ban me!"
  ...
  foreach pnick $pnicks {
   if {[string match "$ban" "$pnick![getchanhost $pnick $chan]"]} {
    ...
    flushmode $chan
     if {[matchattr $hand |o $chan]} {
       chattr $hand |-ao+d $chan
       putserv "KICK $chan $nick :Don't ban $nick ! - You have lost your op on $chan for that !!"
       if {$msgeuser} {putserv "PRIVMSG $nick :If you want your op back please talk to one of my masters. Masters on channel $chan are now: [chanlist $chan m|m]"}
     } else {
       if {[validuser $hand]} {chattr $hand |-ao+d $chan}
       putserv "KICK $chan $nick :Do not ban $nick !"
