#################################################
#Tova e Tcl napraven za da mojete po-lesno da   #
#zarejdate tcl-ite si direktno ot partyline     #
#Komandite sa slednite:                         #  
#.loadtcl <script> || .unloadtcl <script>       #
#.viewtcls <all/loaded> || .tclhelp             #
#################################################                                               
#Tuk ostavqte stoinost za deistvieto na bot-a sled zarejdane na tcl  0 = bez rehash/restart, 1 = rehash, 2 = restart
set ul(refresh) 1
set ul(dir) "scripts"
set ul(pattern) "*.tcl"
#############################################End#######################################################################
...
proc dcc:loadtcl { hand idx arg } {
  ...
  if {![file exists scripts/$arg]} {
    putdcc $idx "-=!Error!Can't find tcl $arg.!Error!=-"
    return
  }
...
proc dcc:unloadtcl { hand idx arg } {
  ...
  close $file
  if {![info exists foo]} {
    putdcc $idx "Не мога да намеря файл: $arg"
    return
  }
  ...
  if {$ul(refresh) == 1} {
    ...
    putdcc $idx "Предлагам да ползвате .restart вместо .rehash"
  }
  return 1
}
...
proc dcc:viewtcls { hand idx arg } {
  ...
  if {[string tolower $arg] == "loaded"} {  
    putlog "#$hand# tcls $arg"
    if {![info exists loaded_scripts]} {
      putdcc $idx "Няма заредени файлове"
    } {
      putdcc $idx "-=<(Списък с файловете)>=-"
	  putdcc $idx "========================="
      foreach script $loaded_scripts {
        putdcc $idx "14Инсталиран4: $script"
      }
    }
    return
    ...
    foreach script $tclfiles {
      if {[lsearch -exact "$loaded_scripts" [string range $script [expr [string length $ul(dir)]+1] end]] == -1} {
        ...
        putdcc $idx "  [string range $script [expr [string length $ul(dir)]+1] end] (зареден)"
      }
    }
    return 1
  }
putdcc $idx "Ползвай: .tcls <loaded/all>"
return
}
putlog  "Инсталиран: load.tcl"
