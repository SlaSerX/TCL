...
# Za da go upravlqva i ima dostap user do komandata trqbva da ima flag +V 
# Usage: .dellvhost spoof/vhost example: .dellvhost test.com
# Usage: .addvhost ident@host password vhost example: .addvhost 
# *ident@83.43.56.45 pass123 spoof.com
# Location ot ircd's vhosts.conf - bot must be on the same shell, or have access to it
set vhostfile "/home/ircd/docs/spoofs.conf"

# Location of bot's vhost data file - it will be created on first run
set vdbfile "/home/ircd/eggdrop/scripts/vhdb.dat"
...
set vhostexpnotifyaddrlist "sAw@mIRC.bg"
...
proc vlistrefresh { } {
		   ...
		   while { $vlistrefresh_con < [llength $vlistrefresh_vhostlist] } {
			  if { [set vlistrefresh_dbfoundnum [lsearch -regexp $vhostlist "^[lindex [split [lindex $vlistrefresh_vhostlist $vlistrefresh_con] \ ] 
0]\[\ \]1"]] > -1 } {
...
proc vhostentry { vhostentry_host vhostentry_pad vhostentry_text } {
		 ...
		 if { [lindex [split $vhostentry_text \ ] 5] == "connecting:" && [llength [split $vhostentry_host @]] == 1 } {
			 ...
			 set vhostlist [lreplace $vhostlist $vhostentry_dbindex $vhostentry_dbindex "$vhostentry_cnhost [clock seconds] [expr [lindex [split 
[lindex $vhostlist $vhostentry_dbindex] \ ] 2] + 1]"]
			}
		    }
		}
...
bind dcc V addvhost vhostadd
bind dcc V dellvhost vhostdell
